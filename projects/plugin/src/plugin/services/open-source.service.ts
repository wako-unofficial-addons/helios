...
export class OpenSourceService {
  ...
  async openStreamLinkSource(streamLinkSource: StreamLinkSource, sourceQuery: SourceQuery, kodiOpenMedia: KodiOpenMedia, action: 'default' | 'more' = 'default') {
    ...
    const actions = action === 'default' ? [settings.defaultPlayButtonAction] : settings.availablePlayButtonActions;
    ...
    settings.availablePlayButtonActions.forEach(action => {
      ...
      switch (action) {
        ...
        case 'open-kodi':
          buttonOptions.cssClass = 'kodi';
          buttonOptions.handler = () => {
            this.openKodi([playlistVideo.url], null, null, null, playlistVideo.openMedia, seek);
          };
          break;
