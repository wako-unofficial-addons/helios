...
import { getScoreMatchingName, getSourcesByQuality, sortTorrentsByPackage, sortTorrentsBySize } from '../tools';
...
export class CachedTorrentSourceService {
  ...
  getStreamLink(source: StreamLinkSource, sourceQuery: SourceQuery) {
  ...
  getBestSource(streamLinkSources: StreamLinkSource[], sourceQuery: SourceQuery, lastPlayedSource?: LastPlayedSource): Observable<StreamLinkSource> {
      ...
      bestSourceObss.push(
        of(true).pipe(
          switchMap(() => {
            ...
            return this.getStreamLink(source, sourceQuery).pipe(
              catchError((e) => {
                return of(null);
              }),
              map(streamLinks => {
